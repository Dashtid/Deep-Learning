...
def deconv_block(base, conc_layer, layer, batch_norm, dropout, dr_rate):
    layer_conv_transpose = Conv2DTranspose(filters=base, kernel_size=(3, 3), strides=(2, 2), padding='same')(layer)
...
def deconv_block_lstm(base, conc_layer, layer, batch_norm, dropout, img_size):
    ...
    x2 = Reshape(target_shape=(1, np.int32(img_size), np.int32(img_size), base))(layer_conv_transpose)
...
def get_unet_deep_sd(base, img_size, img_ch, batch_norm, dropout, dr_rate):
    layer_inp = Input(shape=(img_size, img_size, img_ch))
    layer_b1 = conv_block(base, layer_inp, batch_norm)
    ...
    if dropout:
        layer_d4 = SpatialDropout2D(dr_rate)(layer_mp4)
        layer_b5 = conv_block(base * 16, layer_d4, batch_norm)
    else:
        layer_b5 = conv_block(base * 16, layer_mp4, batch_norm)
    ...
    layer_db1 = deconv_block_sd(base * 16, layer_b5, layer_b6, batch_norm, dropout, dr_rate)

    layer_db2 = deconv_block_sd(base * 8, layer_b4, layer_db1, batch_norm, dropout, dr_rate)

    layer_db3 = deconv_block_sd(base * 4, layer_b3, layer_db2, batch_norm, dropout, dr_rate)

    layer_db4 = deconv_block_sd(base * 2, layer_b2, layer_db3, batch_norm, dropout, dr_rate)

    layer_db5 = deconv_block_sd(base, layer_b1, layer_db4, batch_norm, dropout, dr_rate)
...
def deconv_block_lstm_sd(base, conc_layer, layer, batch_norm, dropout, img_size, dr_rate):
    layer_conv_transpose = Conv2DTranspose(filters=base, kernel_size=(3, 3), strides=(2, 2), padding='same')(layer)
...
def get_unet_deep_lstm_sd(base, img_size, img_ch, batch_norm, dropout, dr_rate):
    layer_inp = Input(shape=(img_size, img_size, img_ch))
    layer_b1 = conv_block(base, layer_inp, batch_norm)
    ...
    layer_db1 = deconv_block_lstm_sd(base * 16, layer_b5, layer_b6, batch_norm, dropout, img_size / 16, dr_rate)

    layer_db2 = deconv_block_lstm_sd(base * 8, layer_b4, layer_db1, batch_norm, dropout, img_size / 8, dr_rate)

    layer_db3 = deconv_block_lstm_sd(base * 4, layer_b3, layer_db2, batch_norm, dropout, img_size / 4, dr_rate)

    layer_db4 = deconv_block_lstm_sd(base * 2, layer_b2, layer_db3, batch_norm, dropout, img_size / 2, dr_rate)

    layer_db5 = deconv_block_lstm_sd(base, layer_b1, layer_db4, batch_norm, dropout, img_size, dr_rate)
